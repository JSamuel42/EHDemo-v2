...
function DossierDetailContent() {
  ...
  const reload = useCallback(() => {
    ...
    if (!s) { router.replace(moduleHref(productId, 'dossier-builder')); return; }
    setSummary(s);
    setSections(getDossierSections(dossierId));
  }, [dossierId, router, productId]);
    ...
    <>
      <div className="px-8 py-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h1 className="font-sans text-3xl text-serif-foreground">{summary.title}</h1>
...
/**
 * This nested [id] route sits outside the generic `[module]/page.tsx`
 * dispatcher (which can't express a third path segment), so it replicates
 * that route's status/wired gating here — otherwise `/p/alnyx/dossier-builder/*`
 * would bypass Alnyx's coming-soon placeholder entirely.
 */
export default function DossierDetailPage() {
  const { productId } = useProduct();
  const status = getModuleStatus(productId, 'dossier-builder');
  const mod = getModule(productId, 'dossier-builder');

  if (status !== 'live' || !mod) {
    const categoryLabel = mod ? CATEGORIES.find(c => c.id === mod.categoryId)?.label ?? '' : '';
    return (
      <ComingSoon
        variant="coming-soon"
        moduleLabel={mod?.label ?? 'Dossier Builder'}
        categoryLabel={categoryLabel}
        accentToken={getProduct(productId).accentToken}
      />
    );
  }

  if (!isModuleWired(productId, 'dossier-builder')) {
    const categoryLabel = CATEGORIES.find(c => c.id === mod.categoryId)?.label ?? '';
    return (
      <ComingSoon
        variant="in-progress"
