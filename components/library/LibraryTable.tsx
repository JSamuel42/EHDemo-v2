...
/** Free-text Article fields editable inline in admin mode (iStent only). */
type EditableField =
  | 'title'
  | 'product_display'
  | 'authors'
...
interface Props {
   ...
   *  dossier in the active asset's portfolio (iStent: Global / UK / Germany).
   *  Empty for Alnyx, so the table renders exactly as before. */
  ...
  /** Admin mode (iStent) — editable cells become inline-editable and dossier
...
/** Renders a dossier cell (read-only): the article's linked section numbers as
 *  mono pills, or an em-dash when the article isn't linked in that dossier. */
                    ...
                    return (
                      <td key={col.id} className={cellBase} style={{ width: 110 }}>
                        {isAdminMode ? (
                          ...
                          <DossierSectionCell numbers={nums} />
                        )}
                      </td>
                    );
                  })}
