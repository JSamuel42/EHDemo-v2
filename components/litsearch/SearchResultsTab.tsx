...
import { ISTENT_ARTICLES as ARTICLES, type Article } from '@/lib/library/istent-data'
import { useLibraryStore, type PushResult } from '@/lib/library/istent-store'
import { DEMO_PUSH_ARTICLES } from '@/data/istent/demo/litSearchExample'
        ...
        <span className="font-mono text-[10px]" style={{ color: 'var(--serif-muted-foreground)' }}>
          {pushResult ? (
            <>
              ...
              {' · '}
              <Link href={moduleHref(productId, 'library')} className="underline hover:opacity-70" style={{ color: '#083860' }}>
                View in Library ↗
              </Link>
            </>
          ) : (
